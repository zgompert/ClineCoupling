## script to format the Motacilla data and fit the cline model
## load libraries
library(data.table)
library(rstan)
...
gdat<-fread("wagtails.loci.012",header=FALSE)
dim(gdat)
#[1] 82 10634326
Gint<-as.matrix(gdat)
Gint[Gint==-1]<-NA
...
ids<-read.table("wagtails.ind.txt",header=FALSE)
...
dp<-abs(P1-P2)
sum(dp > .3,na.rm=TRUE) ## 783313
## also filter by missing, we have enough
pctMiss<-apply(is.na(Gint),2,mean)
sum(dp > .3 & pctMiss < .3,na.rm=TRUE) #783313, none filtered
...
## skewed, almost all < .5, but not mostly 0
...
save(list=ls(),file="clines_Motacilla.rdat")
...
#     50%       5%      95% 
#2.302953 2.190311 2.405608 
quantile(oo$sdrc,probs=c(.5,.05,.95))
#      50%        5%       95% 
#0.1658144 0.1602765 0.1716358 
