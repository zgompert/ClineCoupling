## script to format the Fundulus cossatot data and fit the cline model
## load libraries
library(data.table)
library(rstan)
...
## read main data file, 
gdat<-read.table("Cossatot_genotypes.txt",header=FALSE)
Gint<-t(gdat[,-1]) ## get rid of snp name and transpose

## read ids, ID and parent/hybird, in genotype order
ids<-read.table("Fundulus_pop_desig_Crossatot.csv",header=FALSE,sep=",")
#ids<-ids[,-1]
#ids<-unique(ids)
...
P1<-apply(Gint[ids[,3]=="P1",],2,mean,na.rm=TRUE)/2
P2<-apply(Gint[ids[,3]=="P2",],2,mean,na.rm=TRUE)/2

## ancestry informative
dp<-abs(P1-P2)
sum(dp > .3,na.rm=TRUE) ## 9309
# max missing ~20%
...
save(list=ls(),file="clines_Fundulus_coss.rdat")
...
#     50%       5%      95% 
#0.2249902 0.2048758 0.2468328 
quantile(oo$sdrc,probs=c(.5,.05,.95))
#      50%        5%       95% 
#0.08583818 0.08183980 0.09006004 
