...
gdat<-read.table("Cb.s62.r0.05.maf0.02.het0.5.str.tsv.unlinked",header=FALSE,skip=1)
dim(gdat)
#[1] 536 1339 

## 0 = NA other numbers alleles, no more than 2 per locus
gdat[gdat==0]<-NA
gdat1<-gdat[seq(1,536,2),-c(1:2)]
gdat2<-gdat[seq(2,536,2),-c(1:2)]
...
ids<-read.table("Mytilius_pop_desig.csv",sep=",")
oids<-unique(gdat[,1])
## same order but format on 4 different
...
save(list=ls(),file="clines_Mytilus.rdat")
...
#     50%       5%      95% 
#0.9791549 0.7724579 1.1937009 
quantile(oo$sdrc,probs=c(.5,.05,.95))
#      50%        5%       95% 
#0.06994826 0.06240977 0.0781856 
