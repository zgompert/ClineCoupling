## script to format the Sceloporus data and fit the cline model
## load libraries
library(data.table)
library(rstan)
...
## read main data file, structure format
gdat<-read.table("unlinked_snps.str",header=FALSE)
dim(gdat)
#[1] 514 756 

## 0 = NA other numbers alleles, no more than 2 per locus
gdat[gdat==-9]<-NA
gdat1<-gdat[seq(1,514,2),-c(1,2)]
gdat2<-gdat[seq(2,514,2),-c(1,2)]
low<-as.numeric(apply(gdat[,-c(1,2)],2,min,na.rm=TRUE))
hi<-as.numeric(apply(gdat[,-c(1,2)],2,max,na.rm=TRUE))
Gint<-gdat1
Gint[gdat1!=gdat2]<-1
...
ids<-read.table("Scelop_pop_desig.csv",sep=",")
oids<-unique(gdat[,1])
data.frame(oids,ids[,1])
...
dp<-abs(P1-P2)
sum(dp > .3,na.rm=TRUE) ## 38 
pctMiss<-apply(is.na(Gint),2,mean)
sum(dp > .3 & pctMiss < .3,na.rm=TRUE) #38

anc<-which(dp > .3 & pctMiss < .3)
plot(P1[anc],P2[anc])

hyb<-which(ids[,2]=="H")
length(hyb)## 199 hybrids
...
save(list=ls(),file="clines_Sceloporus.rdat")
...
#     50%       5%      95% 
#0.5450855 0.4048976 0.7941581  
quantile(oo$sdrc,probs=c(.5,.05,.95))
#      50%        5%       95% 
#0.2215370 0.2040250 0.2385873 
