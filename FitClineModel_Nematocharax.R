## script to format the Nematocharax data and fit the cline model
## load libraries
library(data.table)
library(rstan)
...
gdat<-read.table("genomic_unlinked_loci.str",header=FALSE)
dim(gdat)
#[1] 110 1269 

## -9 = NA other numbers alleles, no more than 2 per locus
gdat[gdat==-9]<-NA
gdat1<-gdat[seq(1,110,2),-1]
gdat2<-gdat[seq(2,110,2),-1]
...
ids<-read.table("Nematocharax_pop_desig.txt",header=FALSE)
mean(unique(gdat[,1])==ids[,1])
...
dp<-abs(P1-P2)
sum(dp > .3,na.rm=TRUE) ## 270 
pctMiss<-apply(is.na(Gint),2,mean)
sum(dp > .3 & pctMiss < .3,na.rm=TRUE) #52

anc<-which(dp > .3 & pctMiss < .3)
plot(P1[anc],P2[anc])

hyb<-which(ids[,2]=="H")
length(hyb)## 41 hybrids
...
save(list=ls(),file="clines_Nematocharax.rdat")
...
#     50%       5%      95% 
#1.854102 1.239098 2.407322  
quantile(oo$sdrc,probs=c(.5,.05,.95))
#      50%        5%       95% 
#0.1764848 0.1534069 0.2006976 
