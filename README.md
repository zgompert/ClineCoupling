...
The main R script for running these analyses is [fitGeoClines.R](fitGeoClines.R). This uses the hierarchical Bayesian model defined in [geoclinemod.stan](geoclinemod.stan)[geoclinemod.stan], which we fit with Hamiltonian Monte Carlo using `rstan` (version XX) (the script also includes a non-Bayesian version of the analysis that I played with first). This was done with `R` version 4.1.3. For each data set, I fit the cline models with 20 chains using a 1000 iteration warmup and 1200 iterations total. I then extracted the mean, standard deviation and coefficient of variation of the cline widths, and the actual hierarchical mean and standard deviation on the logit scale (gradient for logit $p$)
