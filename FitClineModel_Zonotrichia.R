## script to format the Zonotrichia data and fit the cline model
## load libraries
library(data.table)
library(rstan)
...
## read main data file, 
gdat<-fread("Zonotrichia_SNPs_6419loci_169ind.csv",header=TRUE,sep=",")
N<-dim(gdat)[1];L<-dim(gdat)[2]-2
...
for(i in 1:L){
	...
	dd<-unlist(gdat[,k])
	dd[dd=="N"]<-NA
	aa<-sort(unique(dd)[unique(dd) %in% c("A","C","G","T")])
	Gint[dd==aa[1],i]<-0
	Gint[dd==aa[2],i]<-2
	het<-unique(dd)[!unique(dd) %in% c("A","C","G","T")]
	het<-het[which(is.na(het)==FALSE)]
	Gint[dd==het,i]<-1
}
...
## nice broad distribution, lots around .5
...
save(list=ls(),file="clines_Zonotrichia.rdat")
...
#      50%        5%       95% 
#0.5670591 0.4902761 0.6598406 
## center, on log scale
quantile(oo$sc,probs=c(.5,.05,.95)) ## median and 90% ETPIs
#      50%        5%       95% 
#0.6302163 0.4987334 0.7888854
## actual SDs, not model parameter and thus not dependent on soft centering
quantile(oo$sdv,probs=c(.5,.05,.95))
#      50%        5%       95%
#0.4202282 0.3727058 0.4667764 
quantile(oo$sdc,probs=c(.5,.05,.95))
#      50%        5%       95% 
#0.6256537 0.5064085 0.7559450

## check soft-centering, not great, but still okay, this is an issue with low coupling I think
...
#     50%       5%      95% 
#2.653592 2.336601 2.941363
quantile(oo$sdrc,probs=c(.5,.05,.95))
#      50%        5%       95% 
#0.1439778 0.1196989 0.1688117
