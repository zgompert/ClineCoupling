## script to format the Aloutatta data and fit the cline model
## load libraries
library(data.table)
library(rstan)
...
## read ids, ID and parent/hybird, only 158 in ids, not all 181
## just use those with good id
ids<-read.table("Alouatta_pop_desig.csv",sep=",")
oids<-read.table("orderedIds.txt")
idsmerge<-data.frame(oids,rep(NA,181))
for(i in 1:181){
	if(idsmerge[i,1] %in% ids[,1]){
		idsmerge[i,2]<-ids[which(ids[,1] == idsmerge[i,1]),2]
	}
} 
ids<-idsmerge ## fixed it
...
save(list=ls(),file="clines_Alouatta.rdat")
...
#     50%       5%      95% 
#0.7022621 0.6218698 0.8007328 
quantile(oo$sdrc,probs=c(.5,.05,.95))
#      50%        5%       95% 
#0.1875515 0.1833285 0.1919232 
