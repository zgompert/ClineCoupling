## script to format the Corvus data and fit the cline model
## load libraries
library(data.table)
library(rstan)
...
## read main data file, genotype data in structure format 
...
gdat[gdat==0]<-NA
gdat1<-gdat[seq(1,124,2),-1]
gdat2<-gdat[seq(2,124,2),-1]
...
ids<-read.table("crow_pop_desig.csv",header=FALSE,sep=",")
oids<-unique(gdat[,1])
...
save(list=ls(),file="clines_Corvus.rdat")
