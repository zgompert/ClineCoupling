## script to format the Gryllus data and fit the cline model
## load libraries
library(data.table)
library(rstan)
...
## read genetic data 
gdat<-read.csv("CT_hybridzone.csv",header=FALSE)
...
## bimodal
write.table(t(round(apply(hi[[1]],2,quantile,probs=c(.5,.025,.975)),5)),file="hi_CT.txt",row.names=F,col.names=F)
...
#     50%       5%      95% 
#0.6959851 0.6152937 0.8072749  
quantile(oo$sdrc,probs=c(.5,.05,.95))
#      50%        5%       95% 
#0.1770208 0.1698420 0.1836951
