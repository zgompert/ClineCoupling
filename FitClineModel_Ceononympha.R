## script to format the Ceononympha data and fit the cline model
## load libraries
library(data.table)
library(rstan)
...
gdat<-read.table("Coenonympha_1SNPrandom.gen")
## looks like genotypes are 2/2, 2/4, or 4/4
Gint<-matrix(NA,nrow=dim(gdat)[1],ncol=dim(gdat)[2]-2)
Gint[gdat[,-c(1,2)]=='2/4']<-1
Gint[gdat[,-c(1,2)]=='2/2']<-0
Gint[gdat[,-c(1,2)]=='4/4']<-2

## read ids, ID and parent/hybird, order matches genotype
ids<-read.table("Coenonympha_pop_desig.csv",sep=",",header=FALSE)
pops<-read.table("pops.txt",header=F)
## PCA sanity check, by pop bc of missing data
P<-matrix(NA,nrow=14,ncol=dim(Gint)[2])
for(i in 1:794){
	P[,i]<-tapply(Gint[,i],INDEX=pops[,1],mean,na.rm=TRUE)/2
}
pco<-prcomp(P[,which(is.nan(apply(P,2,mean))==FALSE)],center=TRUE,scale=FALSE)
#plot(pco$x[,1],pco$x[,2])
# structure not super clear in terms of parents and hybrids, but maybe that is all the missing data
...
dp<-abs(P1-P2)
sum(dp > .3,na.rm=TRUE) ## 81 ancestry informative snps

anc<-which(dp > .3)
plot(P1[anc],P2[anc])

hyb<-which(ids[,2]=="H")
length(hyb)## 93 hybrids
...
#     50%       5%      95% 
#1.069324 0.775106 1.492008
quantile(oo$sdrc,probs=c(.5,.05,.95))
#      50%        5%       95% 
#0.2239909 0.2116491 0.2366191 
